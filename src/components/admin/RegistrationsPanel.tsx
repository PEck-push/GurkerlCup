...
export default function RegistrationsPanel() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (res.ok) setRegs((prev) => prev.filter((r) => r.id !== id));
      else {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <a href="/api/admin/export" className="btn-gold px-5 py-2.5 rounded-full text-sm whitespace-nowrap">
          ⬇ Excel exportieren
        </a>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-[#1E4028]">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {filtered.map((r, i) => (
                <tr key={r.id} className="border-t border-[#1E4028]/60 hover:bg-white/[0.02] transition-colors">
