...
export default function LeaderboardView({
  ...
  const rest: ListEntry[] = entries.filter((e) => e.rank >= 9);

  return (
    <div className="w-full h-full flex flex-col items-center px-[3vw] py-[1.5vh] overflow-hidden">
      <BeamerHeading title={title} accent={accent} variant={variant} className="flex-none mb-[1.2vh]" />
      ...
      {mid.length > 0 && (
        <div className="flex-1 min-h-0 w-full max-w-3xl flex flex-col justify-center gap-[0.8vh]">
          {mid.map((e, i) => (
            <motion.div
              key={e.team.id}
